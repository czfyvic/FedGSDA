...
class GlobalGCN(nn.Module):
    ...
    def test(self, globalParam, epoch, iteration):
        ...
        for pid, param in enumerate(list(self.globalModel.parameters())):
            param.data = torch.tensor(globalParam[pid+5], dtype=torch.float32) #+7
        ...
        if epoch == iteration - 1:
            ...
            dataframe.to_csv('cora/Community3/cc/cora_adjtest_temp_1_c_scatter.csv', index=False, sep=',')

            plt.scatter(x_tsne[:, 0][idx_test], x_tsne[:, 1][idx_test], c=preLabel[idx_test], label="t-SNE")
            fig.savefig('cora/Community3/cc/cora_adjtest_temp_1_c_scatter.png')
            plt.show()
...
class ClientGCN(nn.Module): #
    def __init__(self, data: dict,
        ...
        # self.centers = torch.nn.Parameter(torch.randn(data["num_classes"], hid_channels))
        # self.centerPW = torch.nn.Parameter(torch.rand(hid_channels, hid_channels))
        self.relate = torch.nn.Parameter(torch.rand(hid_channels, hid_channels))
        self.act = nn.Sigmoid()
        self.intratc = 0.5
        self.intertc = 0.5#0.5
        # self.cls_sample_tc = 0.2
        # self.cls_inter_tc = 0.2

        # self.globalKModel = GCNT0(data["dim_features"], 16, data["num_classes"], temperature=0.2)
    ...
    def globalFeatureShiftCal(self, x):
        h = torch.mm(x, self.projectW)
        relate = h @ self.globalRelate @ h.T
        relate = relate / 1000
        # relate = relate.detach().numpy()
        relate = F.softmax(relate / 1000, dim=1)
        # relate = relate.sum(dim=1, keepdim=True)
        # relate = relate.detach().numpy()
        xx = relate @ x
        xx = xx.sum(dim=0, keepdim=True)
        a = 0.000001
        X = x + a * xx
        return X
    ...
    def getLocalTrainOut(self):
        ...
        sampleFlag = False
        if sampleFlag:
            alignedX = self.globalFeatureShiftCal(self.X)
        else:
            alignedX = self.X
        outs = self.localSModel(alignedX, self.A.L_GCN.to_dense())#self.A.L_GCN.to_dense()
        return outs
    ...
    def trainNet(self, localOut, clientId, epoch):
        ...
        if epoch == 3999:
            filePath = 'cora/Community3/'
            localAdj = self.A.L_GCN.to_dense()
            auggAdj = self.augAdj.detach().numpy()
            saveName = filePath + "client" + str(clientId)
            fileLocalAdjPath = saveName + "0.05-localAdj.npy"
            # data = np.load(fileLocalAdjPath)
            np.save(fileLocalAdjPath, localAdj)
            fileAugAdjPath = saveName + "0.05-augAdj.npy"
            np.save(fileAugAdjPath, auggAdj)

        #
        outg1 = self.globalCModel(self.X, self.augAdj)
        outl, lbls = localOut[1][self.train_idx], self.lbls[self.train_idx]

        # centerLoss = self.getCenterContrastLoss(localOut[0][self.train_idx], lbls)
        # intraCenterLoss, interCenterLoss = self.getIntraAndInterCenterContrastLoss(localOut[0][self.train_idx], lbls)


        # centerLoss = self.getLocalAndGlobalCenterContrastLoss(localOut[0][self.train_idx], lbls, 0.0001, 0.5)
        # centerLoss = self.getCenterLoss(localOut[5][self.train_idx], lbls, 0.0001, 0.5)

        # centerLoss = self.localSModel.getCenterLoss(localOut[0][self.train_idx], lbls)
        # crossLoss0 = self.globalKModel.getNeiContrastLoss(localOut[0][self.train_idx], outg0[0],
        #                                             self.globalknnAdj,
        #                                             self.train_idx)

        # crossLoss0 = self.globalKModel.getCrossLoss(localOut[0][self.train_idx], outg0[0][self.train_idx],
        ...
        # self.loss = class_loss + a * centerLoss + b * crossLoss0 + c * crossLoss1
        # self.loss = class_loss + a * centerLoss + b * crossLoss0
        # self.loss = class_loss + a * centerLoss + c * crossLoss1
        # self.loss = class_loss + a * centerLoss + c * crossLoss1
        # self.loss = class_loss0 + a * intraCenterLoss + b * interCenterLoss
        # self.loss = class_loss0 + a * centerLoss
        # self.loss = class_loss0 + b * crossLoss1 + c * genAdj_loss
        self.loss = class_loss0 + class_loss1 + b * crossLoss1 + c * genAdj_loss
        # self.loss = class_loss0 + class_loss1 + a * centerLoss + b * crossLoss1 + c * genAdj_loss
        # self.loss = class_loss0 + class_loss1 + a * intraCenterLoss + d * interCenterLoss +\
        #             b * crossLoss1 + c * genAdj_loss
        # self.loss = class_loss0
    ...
    def getCenterLoss(self, x, label, a0, a1):
        ...
        for i in range(data["num_classes"]):
            ...
            for j in range(data["num_classes"]):
                if i != j:
                    if itemCount == 0:
                        otherCenters = torch.unsqueeze(self.centers[j], dim=0)
                    else:
                        curC = torch.unsqueeze(self.centers[j], dim=0)
    ...
    def getLocalAndGlobalCenterContrastLoss(self, x, label, a0, a1):
        #-------------intra------------------#
        batch_size = x.size(0)
        currentCenter = self.localCenters[label]
        # print(self.centers)

        ###############based on similarity##########################
        posSim = torch.cosine_similarity(x, currentCenter, dim=1)
        posSim = torch.exp((posSim / self.cls_sample_tc) / 100)

        eachX = x[0].repeat([data["num_classes"], 1])
        negSim = torch.cosine_similarity(eachX, self.localCenters, dim=1)
        negSim = negSim.sum(dim=0, keepdim=True)

        for i in range(1, batch_size):
            eachX = x[i].repeat([data["num_classes"], 1])
            eachSim = torch.cosine_similarity(eachX, self.localCenters, dim=1)
            eachSim = eachSim.sum(dim=0, keepdim=True)
            negSim = torch.cat((negSim, eachSim), dim=0)

        negSim = torch.exp((negSim / self.cls_sample_tc) / 100)
        localLoss = (-torch.log(posSim / negSim)).mean()

        #----------inter-------------#
        interPosSim = torch.cosine_similarity(self.localCenters, self.globalCenters, dim=1)
        interPosSim = torch.exp((interPosSim / self.cls_inter_tc) / 100)

        eachInterX = self.localCenters[0].repeat([data["num_classes"], 1])
        interNegSim = torch.cosine_similarity(eachInterX, self.globalCenters, dim=1)
        interNegSim = interNegSim.sum(dim=0, keepdim=True)

        for i in range(1, len(self.localCenters)):
            eachInterX = self.localCenters[i].repeat([data["num_classes"], 1])
            eachInterSim = torch.cosine_similarity(eachInterX, self.globalCenters, dim=1)
            eachInterSim = eachInterSim.sum(dim=0, keepdim=True)
            interNegSim = torch.cat((interNegSim, eachInterSim), dim=0)

        interNegSim = torch.exp((interNegSim / self.cls_inter_tc) / 100)
        globalLoss = (-torch.log(interPosSim / interNegSim)).mean()

        loss = a0 * localLoss + a1 * globalLoss
    ...
    def getIntraAndInterCenterContrastLoss(self, x, label):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            #     negsum = interPosSim[i]  #首项为第一项正例
            #     for j in range(len(self.centers)):
            #         if i != j:
            #            eachInterSim = torch.cosine_similarity(eachInterX, self.centers[j], dim=0)
            #            negsum += eachInterSim
            #     interNegSim.append(negsum)
            # interNegSim = torch.stack(interNegSim)
            #
    ...
    def getCenterContrastLoss(self, x, label):
        """
        Args:
            x: feature matrix with shape (batch_size, feat_dim).
            labels: ground truth labels with shape (batch_size).
        ...
        ###############based on similarity##########################
        posSim = torch.cosine_similarity(x, currentCenter, dim=1)
        posSim = torch.exp((posSim / self.tc)/100)
        ...
        negSim = torch.exp((negSim / self.tc)/100)
        loss = (-torch.log(posSim / negSim)).mean()

        return loss


    def getIterKmeansCenterAndLabels(self, outs_l):
        self.kmeans.fit(outs_l)
        labels = self.kmeans.predict(outs_l)
...
if __name__ == "__main__":
    ...
    # G = load_graph(data["edge_list"])
    # obj = GraphXX()
    # G1 = obj.createGraph(data["edge_list"])
    ...
    dataframe0.to_csv('cora/Community3/cc/3_adjTest_temp_0.05_acc.csv', index=False, sep=',')
    dataframe1.to_csv('cora/Community3/cc/3_adjTest_temp_0.05_f1_acc.csv', index=False, sep=',')
    dataframe2.to_csv('cora/Community3/cc/3_adjTest_temp_0.05_precision_acc.csv', index=False, sep=',')
    dataframe3.to_csv('cora/Community3/cc/3_adjTest_temp_0.05_recall_acc.csv', index=False, sep=',')
